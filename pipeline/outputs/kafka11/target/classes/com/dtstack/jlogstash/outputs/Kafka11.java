...
/**
 * @author: haisi
 * @date 2019-06-25 10:03
 */
public class Kafka11 extends BaseOutput {

    private static final Logger logger = LoggerFactory.getLogger(Kafka11.class);
    ...
    protected void emit(Map event) {
        try {
            while (!isKafkaHealth.get()){
                Thread.sleep(4000L);
                logger.warn("kafka11 is unhealthy, producing is stopped");
            }
            ...
            logger.error("kafka11 output error to block", e);
            try {
    ...
    public void prepare() {
        try {
            ...
            if (!"".equals(bootstrapServers.trim())){
            ...
            logger.error("kafka11 producer init error", e);
            System.exit(1);
        }
    }
    ...
    public void release() {
        producer.close();
        logger.warn("output kafka11 release.");
    }
